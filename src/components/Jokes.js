...
export default function Jokes() {
  ...
  const [jokesArray] = useState([
    ...
    },
    {
      ...
      resposta:
        "Porque quando ele bate e respondem que tem gente, ele diz em seguida: Shake-espera."
    },
    {
      pergunta:
    ...
    {
      pergunta:
        "Um americano perguntou para o bom velhinho do natal: Papai, o senhor está bem?",
  ...
  function changeJoke() {
    let now = Date.now();
    console.log("Joke");
    if (now - joke.time > 30000) {
      joke.time = Date.now();
      setJoke(jokesArray[getRandomInt(0, jokesArray.length - 1)]);
    }
    setTimeout(() => {
      changeJoke();
    }, 31000);
